...
================================================================================
==== Expected Program slots ====
 Required: componentToCopy (BOrd), copyTo (BOrd), destinationMode (BStatusEnum), keepAllLinks (boolean/BBoolean), Dry Run? (boolean), ResultsCsvDestination/ResultsToCsvDestination (BOrd)
 Notes: BqlDestinationColumn (int) is used in BQL mode to select the destination column.
*/
...
private int getDestModeOrdinal() { final BStatusEnum m = getDestinationMode(); final String s = (m != null) ? m.toString() : "Direct"; final String tag = (s != null) ? s.trim() : "Direct"; if ("BQL".equalsIgnoreCase(tag)) return 1; if ("CSV".equalsIgnoreCase(tag)) return 2; if ("Direct".equalsIgnoreCase(tag)) return 0; return -1; }
...
private boolean resolveDryRun(final java.util.Map<String,Object> cfg) { if (asBool(cfg.get("dryRun"))) return true; try { java.lang.reflect.Method[] methods = this.getClass().getMethods(); for (int i = 0; i < methods.length; i++) { java.lang.reflect.Method m = methods[i]; String name = m.getName().toLowerCase(); if (m.getParameterTypes().length == 0 && name.indexOf("dry") >= 0 && name.indexOf("run") >= 0) { Class<?> rt = m.getReturnType(); Object v = m.invoke(this); if (rt == Boolean.TYPE || rt == Boolean.class) return ((Boolean) v).booleanValue(); if (v instanceof BBoolean) return ((BBoolean) v).getBoolean(); } } } catch (Throwable ignore) {} return false; }
...
private String trimExt(String base) { int i = base.toLowerCase().lastIndexOf(".csv"); return (i > 0) ? base.substring(0, i) : base; }
...
// ===== CSV Row Writing (Timestamp first + version tag) =====
...
private String safe(final String s) { return (s == null) ? "" : s; }


private void appInfo(final String msg) { try { System.out.println("[MultiLocationCopier] " + msg); } catch (Throwable ignore) {} }
...
private String getSourceNameSafe() { try { BOrd o = getComponentToCopy(); if (o != null) { BObject bo = o.get(); BComponent c = (bo != null) ? bo.asComponent() : null; if (c != null) return c.getName().toString(); } } catch (Throwable ignore) {} return "(unknown)"; }
private void preflightCsvWritable(final BOrd csvOrd) { if (csvOrd == null) throw new IllegalArgumentException("resultsCsvDestination .csv is required"); try { final BObject obj = csvOrd.resolve().get(); if (obj == null) throw new IllegalArgumentException("resultsCsvDestination did not resolve"); java.io.OutputStream os = null; try { java.lang.reflect.Method m = obj.getClass().getMethod("openOutputStream", new Class[]{boolean.class}); os = (java.io.OutputStream) m.invoke(obj, Boolean.TRUE); } catch (Throwable ignore) {} if (os == null) { try { java.lang.reflect.Method m = obj.getClass().getMethod("getOutputStream", new Class[]{boolean.class}); os = (java.io.OutputStream) m.invoke(obj, Boolean.TRUE); } catch (Throwable ignore) {} } if (os == null) { try { java.lang.reflect.Method m = obj.getClass().getMethod("openOutputStream", new Class[0]); os = (java.io.OutputStream) m.invoke(obj, new Object[0]); } catch (Throwable ignore) {} } if (os == null) { try { java.lang.reflect.Method m = obj.getClass().getMethod("getOutputStream", new Class[0]); os = (java.io.OutputStream) m.invoke(obj, new Object[0]); } catch (Throwable ignore) {} } if (os == null) throw new IllegalStateException("resultsCsvDestination is not writable"); try { os.close(); } catch (Throwable ignore) {} appInfo("Results CSV ready: " + csvOrd.toString()); } catch (Throwable t) { throw new IllegalStateException("Results CSV not ready: " + safe(t.getMessage())); } }
private void appBannerStart(final String mode, final String srcName, final String resultsPath) { if (!bannerPrinted) { appInfo("v5.15 — F. Lacroix 12/18/2025 — Direct/BQL/CSV"); appInfo("Mode: " + mode + " | Source: " + srcName + (resultsPath != null ? " | Results: " + resultsPath : "")); bannerPrinted = true; } }
// ===== Copy params =====
private BComponent makeParamsWithKeepLinks() { final BComponent params = new BComponent(); final BBoolean keepLinksVal = coerceKeepAllLinks(); params.add("keepAllLinks", keepLinksVal); return params; }
...
private BOrd buildDestinationOrd(final String cellValue) { final String v = (cellValue == null) ? "" : cellValue.trim(); if (v.isEmpty()) return null; if (v.startsWith("station:")) return BOrd.make(v); if (v.startsWith("slot:/")) return BOrd.make("station:|" + v); if (v.startsWith("/")) return BOrd.make("station:|slot:" + v); return BOrd.make("station:|slot:/" + v); }
